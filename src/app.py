...
def main():
            ...
            try:
                ...
                if card_data and card_data.get("fields"):
                    with st.spinner("✅ Validating extracted data..."):
                        ...
                        # Update card_data with validated fields
                        card_data["validated_fields"] = validation_results["validated_fields"]
                        card_data["invalid_fields"] = validation_results["invalid_fields"]
    ...
    with col3:
        if card_data and main_portrait_path:
            # Prepare JSON data with validated fields
            validated_fields = card_data.get("validated_fields", card_data.get("fields", {}))
            json_data = {
                "card_type": card_data["card_type"],
                "card_type_confidence": round(card_data["card_type_confidence"], 2),
                "fields": {k: v for k, v in validated_fields.items() if v is not None},
                "raw_fields": {k: v for k, v in card_data.get("fields", {}).items() if v is not None},
                "portrait_path": main_portrait_path,
                "validation_summary": validation_results if validation_results else None
            }
            ...
            try:
                ...
                if storage_result["success"]:
                    st.success(f"✅ Data stored (Record ID: {storage_result['record_id']})")
            except Exception as e:
                st.warning(f"⚠️ Could not store in database: {str(e)}")
...
def display_card_info(card_data: dict, show_ocr_text: bool = False):
    ...
    # Show validated fields if available, otherwise raw fields
    validated_fields = card_data.get("validated_fields", {})
    raw_fields = card_data.get("fields", {})
    invalid_fields = card_data.get("invalid_fields", {})
    
    # Use validated fields if available, otherwise raw fields
    display_fields = validated_fields if validated_fields else raw_fields
    non_empty_fields = {k: v for k, v in display_fields.items() if v is not None and str(v).strip()}
    ...
    if non_empty_fields:
        ...
        import pandas as pd
        df = pd.DataFrame([
            {"Field": field, "Value": value, "Status": "✅ Valid" if field in validated_fields else "⚠️ Raw"}
        ...
        if validated_fields:
            ...
            validation_rate = (validated_count / total_fields * 100) if total_fields > 0 else 0
            ...
            if invalid_fields:
                with st.expander("⚠️ Invalid Fields (Require Manual Review)"):
                    for field, error in invalid_fields.items():
                        st.text(f"{field}: {error}")
